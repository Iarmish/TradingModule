...
namespace ShortPeakRobot.Robots.Algorithms
{
    internal class ShortPeakTemp
    {
        ...
        public Peak CurrentHighPeak { get; set; } = new Peak();
        public Peak CurrentLowPeak { get; set; } = new Peak();
        public List<DateTime> InvalidHighPeaks { get; set; } = new List<DateTime>();
        public List<DateTime> InvalidLowPeaks { get; set; } = new List<DateTime>();


        //----------------------------------------------
        public ShortPeakTemp(int robotId, int robotIndex)
        {
            RobotId = robotId;
        ...
        public async void NewTick(RobotCommands command)
        {
            ...
            //================сохраннение недействительных пиков
            if (CurrentHighPeak.Volume != 0 && currentPrice - robot.BaseSettings.OffsetPercent > CurrentHighPeak.Volume)
            {
                InvalidHighPeaks.Add(CurrentHighPeak.Date);
                CurrentHighPeak = new();
            }

            if (lowPeak.Volume != 0 && currentPrice + robot.BaseSettings.OffsetPercent < CurrentLowPeak.Volume)
            {
                InvalidLowPeaks.Add(CurrentLowPeak.Date);
                CurrentLowPeak = new();
            }
        ...
        public void ChartAnalyse()
        {
            ...
            //-скидываем пики по списку недействительных пиков
            if (InvalidHighPeaks.Contains(highPeak.Date))
            {
                highPeak = new();
                CurrentHighPeak = new();
            }

            if (InvalidLowPeaks.Contains(lowPeak.Date))
            {
                lowPeak = new();
                CurrentLowPeak = new();
            }

            InvalidHighPeaks = InvalidHighPeaks.TakeLast(4).ToList();
            InvalidLowPeaks = InvalidLowPeaks.TakeLast(4).ToList();
            ///------------------------------
            var SignalSellPrice = RobotServices.GetSignalPrice(robot.BaseSettings.OffsetPercent, lowPeak.Volume,
                        RobotOrderType.SignalSell, FuturesOrderType.StopMarket);
